...
export default class App extends React.Component {
  constructor(props) {
   ...
   componentDidMount() {
    ...
    const configure =  GoogleSignin.configure({
      ...
      webClientId:
        "1012116442555-3lpg1o4t4m1hu53i582j3q3pt1ia0ibl.apps.googleusercontent.com",
    });
  ...
  _signIn = async () => {
    ...
    try {
      ...
      console.log("State",this.state.userInfo.user.id)
      ...
      const profileInfo = {
        ...
        fullName: this.state.userInfo.user.name,
        email: this.state.userInfo.user.email,
        photo: this.state.userInfo.user.photo,
      };

      this.props.setUserInfo(profileInfo);
      // this.props.navigation.replace('Profile', {userInfo: profileInfo});
      // await GoogleSignin.getTokens().then((re) => {
      //   this.setState({ token: re.accessToken });
      // });
